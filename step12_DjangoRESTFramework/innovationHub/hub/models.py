...
def is_esprit_email(value):
    ...
    if not str(value).endswith("@esprit.tn"):
        raise ValidationError(
            'Votre Email doit être @Esprit.tn', params={'value': value})


class TimeStampedModel(models.Model):
    # Ce champs sera modifié à la création
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(
        auto_now=True)  # modifier pour chaque update
...
class User(models.Model):
    ...
    email = models.EmailField(
        'Email', validators=[is_esprit_email], null=False)
...
class Project(models.Model):
    ...
    def total_allocated_by_members(self):
        ...
        sum_invested_by_members = list_members_in_p.all(
        ).aggregate(Sum('time_allocated_by_member'))
